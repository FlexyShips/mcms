...
export async function sendWaitlistAcknowledgement(input: {
  ...
  const content = `<p>Hi ${name},</p><p>Thanks for your interest in ${env.APP_NAME}. We have added <strong>${htmlEscape(input.email)}</strong> to the waitlist for <strong>${companyName}</strong>.</p><p>We will be in touch when access is available.</p>`;

  await getTransporter().sendMail({
    from,
    to: input.email,
    subject: 'You have been added to the ${env.APP_NAME} waitlist',
    text: `Hi ${input.name},\n\nThanks for your interest in ${env.APP_NAME}. We have added ${input.email} to the waitlist for ${input.companyName}. We will be in touch when access is available.\n\nThe ${env.APP_NAME} team`,
...
export async function sendWaitlistLaunchAnnouncement(input: {
  ...
  const content = `<p>Hi ${name},</p><p>We are excited to let you know that ${env.APP_NAME} has officially launched.</p><p>Thank you for joining the waitlist for <strong>${companyName}</strong>. You can now explore ${env.APP_NAME} and get started.</p><p><a href="${launchUrl}" style="display: inline-block; background: #0f2a43; color: #ffffff; padding: 12px 20px; border-radius: 4px; text-decoration: none;">Visit ${env.APP_NAME}</a></p>`;

  await getTransporter().sendMail({
    from,
    to: input.email,
    subject: `${env.APP_NAME} has officially launched`,
    text: `Hi ${input.name},\n\n${env.APP_NAME} has officially launched. Thank you for joining the waitlist for ${input.companyName}. You can now explore ${env.APP_NAME} at ${launchUrl}\n\nThe ${env.APP_NAME} team`,
    html: emailTemplateWrapper({
      title: `${env.APP_NAME} has launched`,
      content,
    }),
  });
...
export async function sendTenantWelcome(input: {
  ...
  const content = `<p>Hi ${fullName},</p><p>Welcome to ${env.APP_NAME}. Your workspace for <strong>${companyName}</strong> is ready.</p><p><a href="${url}" style="display: inline-block; background: #0f2a43; color: #ffffff; padding: 12px 20px; border-radius: 4px; text-decoration: none;">Open your workspace</a></p>`;

  await getTransporter().sendMail({
    from,
    to: input.email,
    subject: `Welcome to ${env.APP_NAME}`,
    text: `Hi ${input.fullName},\n\nWelcome to ${env.APP_NAME}. Your workspace for ${input.companyName} is ready. Open it here: ${input.url}\n\nThe ${env.APP_NAME} team`,
    html: emailTemplateWrapper({ title: `Welcome to ${env.APP_NAME}`, content }),
  });
...
export async function sendEmailVerificationMail(input: {
  ...
  const companyName = htmlEscape(input.companyName);
  const url = htmlEscape(input.url);
  const content = `<p>Hi ${fullName},</p><p>Please verify your email address to activate your ${env.APP_NAME} workspace for <strong>${companyName}</strong>.</p><p><a href="${url}" style="display: inline-block; background: #0f2a43; color: #ffffff; padding: 12px 20px; border-radius: 4px; text-decoration: none;">Verify your email</a></p><p>If you didn't request this, you can safely ignore this email.</p>`;

  await getTransporter().sendMail({
    from,
    to: input.email,
    subject: `Verify your email for ${env.APP_NAME}`,
    text: `Hi ${input.fullName},\n\nPlease verify your email address to activate your ${env.APP_NAME} workspace for ${input.companyName}.\n\nVerify here: ${input.url}\n\nIf you didn't request this, you can safely ignore this email.\n\nThe ${env.APP_NAME} team`,
    html: emailTemplateWrapper({ title: `Verify your email for ${env.APP_NAME}`, content }),
  });

  logger.info({ email: input.email }, 'Email verification email sent');
}
...
export async function sendUserInvite(input: {
  ...
  const content = `<p>Hi ${firstName},</p><p>You have been invited to join <strong>${companyName}</strong> on ${env.APP_NAME} as a <strong>${role}</strong>.</p><p>Set your password to accept the invitation and access the workspace:</p><p><a href="${inviteUrl}" style="display: inline-block; background: #0f2a43; color: #ffffff; padding: 12px 20px; border-radius: 4px; text-decoration: none;">Accept invitation</a></p><p>This invitation expires in 48 hours.</p>`;

  await getTransporter().sendMail({
    from,
    to: input.email,
    subject: `You have been invited to join ${input.companyName} on ${env.APP_NAME}`,
    text: `Hi ${input.firstName},\n\nYou have been invited to join ${input.companyName} on ${env.APP_NAME} as a ${role}. Accept your invitation and set your password here: ${input.inviteUrl}\n\nThis invitation expires in 48 hours.\n\nThe ${env.APP_NAME} team`,
    html: emailTemplateWrapper({ title: 'You have been invited to ${env.APP_NAME}', content }),
  });
...
export async function sendSignupPayment(input: {
  ...
  const content = `<p>Hi ${fullName},</p><p>Thanks for signing up for ${env.APP_NAME} for <strong>${companyName}</strong>.</p><p>Complete your payment to upgrade to ${input?.plan} in your workspace:</p><p><a href="${checkoutUrl}" style="display: inline-block; background: #0f2a43; color: #ffffff; padding: 12px 20px; border-radius: 4px; text-decoration: none;">Complete payment</a></p>`;

  await getTransporter().sendMail({
    from,
    to: input.email,
    subject: `Upgrade your ${env.APP_NAME} workspace`,
    text: `Hi ${input.fullName},\n\nThanks for signing up for ${env.APP_NAME} for ${input.companyName}. Complete your payment to activate your workspace: ${input.checkoutUrl}\n\nThe ${env.APP_NAME} team`,
    html: emailTemplateWrapper({ title: `Complete your ${env.APP_NAME} signup`, content }),
  });
