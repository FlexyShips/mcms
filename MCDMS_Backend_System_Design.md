...
HR Manager → "Activate Account" on crew member profile
      ...
      ├── Email sent: https://{slug}.${env.FRONTEND_DOMAIN}/accept-invite?token=xxx
      │
...
export const requireModule =
  (module: keyof TenantModules) => async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!JSON.parse(settings as string)[module]) {
      return res.status(403).json({
        message:
          'Your current plan does not include this module. Please upgrade or contact support.',
        module,
      });
    }
    next();
...
```
bluetrack-maritime.${env.FRONTEND_DOMAIN}
oceanfleet.${env.FRONTEND_DOMAIN}
```
...
sudo certbot certonly \
  ...
  -d "mcdms.com" -d "*.${env.FRONTEND_DOMAIN}"
```
...
export async function resolveTenant(req: Request, res: Response, next: NextFunction) {
  ...
  if (!tenant)
    return res
      .status(404)
      .json({ message: `No company found at ${subdomain}.${env.FRONTEND_DOMAIN}` });
...
| SSL (Certbot)    | Wildcard cert `*.${env.FRONTEND_DOMAIN}` | **Once** (auto-renews)  |
...
Super Admin invites company
      ...
      └── Invite email sent: https://{slug}.${env.FRONTEND_DOMAIN}/onboarding?token=xxx
```
    ...
    for (const days of alertDays) {
      ...
      for (const cert of vesselCerts) {
        ...
        for (const recipient of recipients) {
          await notificationQueue.add('send.vessel.cert', {
            tenantId: tenant.id,
            certId: cert.id,
      ...
      for (const doc of crewDocs) {
        ...
        for (const recipient of recipients) {
          await notificationQueue.add('send.crew.doc', {
            tenantId: tenant.id,
            docId: doc.id,
            recipientId: recipient.id,
            daysRemaining: days,
...
| **Logs in at**             | `admin.${env.FRONTEND_DOMAIN}`    | `{slug}.${env.FRONTEND_DOMAIN}`              |
...
POST /api/v1/users/invite
      ...
      └── Email: https://{slug}.${env.FRONTEND_DOMAIN}/accept-invite?token=xxx
...
The Super Admin operates the platform from `admin.${env.FRONTEND_DOMAIN}`. Every tenant can be fully controlled without touching the database directly. All Super Admin actions are logged in `SuperAdminAuditLog` with before/after state.
...
```
PLATFORM SETTINGS (Super Admin — admin.${env.FRONTEND_DOMAIN})
│
│  Controls globally:
...
│
└── TENANT SETTINGS (Tenant Admin — {slug}.${env.FRONTEND_DOMAIN}/settings)
       │
...
async function deliverWebhook(job: Job) {
  ...
  try {
    ...
    await prisma.webhookDelivery.update({
      where: { id: deliveryId },
      data: {
        status: nextDelay ? 'PENDING' : 'EXHAUSTED',
        nextRetryAt: nextDelay ? addSeconds(new Date(), nextDelay) : null,
      },
    });
...
ADMIN_URL=https://admin.${env.FRONTEND_DOMAIN}
```
...
server {
    listen 443 ssl;
    server_name admin.${env.FRONTEND_DOMAIN};
...
server {
    listen 80;
    server_name *.${env.FRONTEND_DOMAIN} mcdms.com;
...
module.exports = {
  ...
  ignorePatterns: ['dist/', 'node_modules/', '*.js', '!.eslintrc.js', '!commitlint.config.js'],
};
```
...
server {
    listen 443 ssl;
    server_name staging.${env.FRONTEND_DOMAIN}
          ...
          script: |
            github.rest.issues.createComment({
              ...
              body: `## ✅ Staging Deployment Successful\n\n` +
                    `**Staging URL:** https://staging.${env.FRONTEND_DOMAIN}\n\n` +
...
5. Open Pull Request → main on GitHub
         ...
         └── GitHub Actions pr-checks.yml fires:
               ...
               └── Job 4: Deploy to staging.${env.FRONTEND_DOMAIN}
                     └── Staging URL posted as PR comment ✅

6. Review changes on staging.${env.FRONTEND_DOMAIN}
...
_End of MCDMS Backend System Design v5.0_
